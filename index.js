...
for (let i=0;i<store.passengers.length;i++){
if (store.passengers[i].id == z[i])
h.push(store.passengers[i])
}
return h
...
return  store.trips.filter(v=>v.passengerId == this.id)
  ...
  drivers(){
    ...
    let h = []
  for (let i=0; i<store.trips.length;i++){
  if (this.id == store.trips[i].passengerId){
  z.push(store.trips[i].driverId)
  }
  }
...
return store.drivers.filter(v=> v.id == this.driverId)
  ...
  passenger(){
return store.passengers.filter(v=> v.id = this.passengerIdid)
